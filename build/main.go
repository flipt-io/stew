				...
				Subcommands: []*cli.Command{
					{
						...
						Action: func(ctx *cli.Context) error {
							return withBase(ctx.Context, func(client *dagger.Client, base *dagger.Container, platform dagger.Platform) error {
								stew, err := image(ctx.Context, client, base, platform)
								if err != nil {
									return err
								}

								_, err = stew.Export(ctx.Context, "stew.tar")
								return err
							})
						},
					},
					{
						...
						Flags: []cli.Flag{
							&cli.StringFlag{
								Name:    "registry",
								Value:   "ghcr.io",
								EnvVars: []string{"STEW_BUILD_REGISTRY"},
							},
							&cli.StringFlag{
								Name:    "username",
								Value:   "flipt-io",
								EnvVars: []string{"STEW_BUILD_USERNAME"},
							},
							&cli.StringFlag{
								Name:    "password",
								Value:   "",
								EnvVars: []string{"STEW_BUILD_PASSWORD"},
							},
							&cli.StringFlag{
								Name:    "image-name",
								Value:   "stew:latest",
								EnvVars: []string{"STEW_BUILD_IMAGE_NAME"},
							},
						},
						Action: func(ctx *cli.Context) error {
							var (
...
func image(ctx context.Context, client *dagger.Client, base *dagger.Container, platform dagger.Platform) (*dagger.Container, error) {
	return client.
		Container(dagger.ContainerOpts{Platform: platform}).
		From("alpine:3.18").
		WithExec([]string{"mkdir", "-p", "/etc/stew"}).
		WithFile("/usr/local/bin/stew", base.File("/usr/local/bin/stew")).
		WithDefaultArgs(dagger.ContainerWithDefaultArgsOpts{
			Args: []string{"/usr/local/bin/stew", "-config", "/etc/stew/config.yml"},
		}).
		Sync(ctx)
}
...
func withBase(ctx context.Context, fn func(client *dagger.Client, base *dagger.Container, platform dagger.Platform) error, opts ...option) error {
	return withClient(ctx, func(client *dagger.Client, platform dagger.Platform) error {
		...
		return fn(client, base.
			...
			WithExec([]string{"go", "build", "-o", "/usr/local/bin/stew", "./main.go"}),
			platform,
		)
	}, opts...)
}
